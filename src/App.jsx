...
export default function App() {
  ...
  const handleAdd = useCallback(async (raw, status) => {
    ...
    } catch (e) { alert('添加失败: ' + (e.message || '未知')); }
  }, [user]);
  ...
  const handleReset = useCallback(async () => {
    if (!user || !confirm('确定清空？不可恢复。')) return;
  ...
  const handleExport = useCallback(() => {
    ...
    const blob = new Blob([JSON.stringify(movies.map(m => ({ id: m.id, title: m.movie?.title, year: m.movie?.year, status: m.status })), null, 2)], { type: 'application/json' });
  ...
  const handleImport = useCallback(async () => {
    ...
    if (!entries.length) { alert('无效分享码'); return; }
  ...
  if (authLoading) {
    return (
      <div className="splash-screen">
        <svg className="splash-logo--large" viewBox="0 0 64 64"><g transform="translate(32,32)">
